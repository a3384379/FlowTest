...
def build_gate_plan(paths: Iterable[str]) -> GatePlan:
    normalized = frozenset(path.strip().removeprefix("./") for path in paths if path.strip())
...
def main() -> int:
    ...
    if not args.repository or not token:
        ...
        return 2
    plan = build_gate_plan(args.paths_file.read_text(encoding="utf-8").splitlines())
