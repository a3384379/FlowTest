...
- 新 V6 代码基线 SHA：`65a427ad0ddcfa6704b7c1138a97faa04b277e53`。
- Migration Head：`20260823_0045`。

H0 已完成且合并后 `main` 全链通过。H0 未新增 V6 运行时能力，未启用任何 V6
feature flag，未创建 Tag、Release、Beta 或 RC。本文档是独立的收口证据变更；它不会
改变上述 V6 代码基线。
...
原 PR #42 中发现旧 `pull_request` 工作流会执行 PR-controlled Gate 代码。该 P1 未被
直接 Resolve，也未通过放宽保护合并。治理引导过程如下：
...
3. `pull_request_target` Controller 只 checkout 可信 base/default-branch 代码；PR Head
   只作为文件列表、SHA 和 child-check 元数据读取。
4. Controller 在 PR Head 或 main SHA 上先写 `Required Gate=pending`，验证选中的 child
   check 名称、workflow path、event、精确 Head SHA 与 GitHub Actions app 后才写最终状态。
5. PR 文件列表同时检查 `filename` 与 `previous_filename`，分页并在 GitHub 上限处
   fail closed；普通 PR 不能修改 `.github/workflows/` 或 `scripts/required_gate.py`。
6. 最终写状态前重新读取 PR base/head；过期事件只能失败。
7. 正式 ruleset `21653796` 已恢复为 `main-required-gate`：Require PR、conversation
   resolution、strict up-to-date、禁止删除和 non-fast-forward，仅要求 GitHub Actions
   app `15368` 签发的 `Required Gate`，无 bypass。
...
H0 的结束条件已满足，`65a427ad0ddcfa6704b7c1138a97faa04b277e53` 可作为
S48 的 V6 代码基线。S48 必须从本文档 closure PR 合并且其 main push Gate 成功之后再开始。
