...
1. 实现 Extract 和 Assert 节点，在 ExecutionContext 中记录结构化输入、输出和来源。
2. 实现 Condition 和 Delay 节点，冻结条件分支、跳过与失败传播语义。
3. 实现 Dataset 节点及 CSV/JSON/Excel 解析，限制 1000 行并以默认并发 5 形成子执行。
4. 实现结构化字段映射，支持 A 响应字段映射到 B 请求，并显示变量来源。
5. 扩展 React Flow 节点库、属性面板、运行着色和测试。
6. 补齐数据驱动、条件分支、跳过解释及变量追踪验收，发布 `v0.3.0`。
