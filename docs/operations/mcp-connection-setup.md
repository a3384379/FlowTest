...
Change Regression 纯分析准备和 Test Plan 更新建议分别使用 `mcp:regression:prepare`、
`mcp:test-plan:propose`；Preview Graceful Cancel 复用 `mcp:preview:execute`，不提供强制取消。
