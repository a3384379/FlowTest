...
浏览器打开 `http://127.0.0.1:8000`，使用账号 `admin`、密码 `admin123456` 登录。Standalone 不强制首次改密；
新建用户和主动修改密码的最低长度为 8 位。默认凭据在各环境中一致，可按需要自行修改。
安装人员不要把 `.env` 上传 GitHub、工单或聊天工具。

### 项目出站策略

进入“项目设置 → 安全策略”即可切换“启用出站安全策略”。Standalone 新建项目默认关闭，因此可以直接
导入和调试 `localhost`、`127.0.0.1` 及公司私网服务；关闭时仍保留协议、端口、凭据、解析和连接安全校验。
开启后恢复严格模式，回环/链路本地/云元数据/保留地址会被阻断，未加入允许域名或私网 CIDR 的目标也会被阻断。
Compact/Full 和旧项目默认开启。该开关同时覆盖文档导入、接口/环境执行、工作流、Webhook 和协议调试。

接口文档选择器和导入结果使用固定高度滚动区域；左侧接口列表支持搜索、HTTP 方法筛选和服务端分页，接口数量
较多时无需长页面下拉。
